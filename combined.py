...
class ScheduleScreen(Screen):
    ...
    def on_check_press(self, instance_table, current_row):
        ...
        dialog = MDDialog(
            ...
            text=f"Delete schedule for {current_row[0]} at {current_row[2]}?",
            buttons=[
                MDFlatButton(
                    text="CANCEL",
                ...
                MDRaisedButton(
                    text="DELETE",
                    md_bg_color=(1, 0, 0, 1),
                    on_release=lambda x: self.delete_selected_schedule(dialog)
                ),
            ],
        )
        dialog.open()

    def delete_selected_schedule(self, dialog):
        if hasattr(self, 'selected_row'):
            if self.selected_row in self.table.row_data:
                self.table.row_data.remove(self.selected_row)
    ...
    def add_schedule(self):
        ...
        if all([subject, building_name, room, day, time_start, time_end]):
            ...
            self.table.row_data.append((subject, building_name, room, day, time_start, time_end))
            self.table.row_data = self.table.row_data.copy()
...
class MapScreen(Screen):
    ...
    def show_markers_for_day(self, day):
        ...
        for entry in schedule_screen.table.row_data:
            if len(entry) >= 6:
                subject, building_name, room, sched_day, time_start, time_end = entry[:6]
            else:
                continue
    ...
    def draw_route_to_building(self, building_name, path):
        ...
        for marker in self.markers:
            if marker.building_name == building_name:
                marker.source = "red_marker_selected.png"
                marker.size = (dp(40), dp(55))
                self.current_selected_building = building_name
            else:
                marker.source = "red_marker.png"
                marker.size = (dp(35), dp(50))
